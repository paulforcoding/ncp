...
- **多后端支持** — 本地文件系统、远程 ncp 服务器（`ncp://`）、阿里云 OSS（`oss://`）、腾讯云 COS（`cos://`）。
...
从一个或多个源复制文件到目标。支持本地、`ncp://`、`oss://` 和 `cos://` 协议。
...
云 URL 通过 userinfo 引用名为 profile 的凭据集:`oss://<profile>@bucket/path/` 或 `cos://<profile>@bucket/path/`。Profile 集中定义在 `ncp_config.json` 的 `Profiles` 字段下。同一 profile 名可以指向不同账号或区域,这正是跨账号迁移的表达方式:
...
- `Provider` 必须等于 URL scheme(`oss://prod@...` 要求 `Profiles.prod.Provider == "oss"`; `cos://prod@...` 要求 `Profiles.prod.Provider == "cos"`)。
...
**约束:**
- OSS 或 COS 参与时,`--cksum-algorithm` 必须为 `md5`(云存储后端用 Content-MD5 校验完整性,不支持 `xxh64`)。
