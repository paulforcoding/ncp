...
- **Agent-First 输出** — 结构化 NDJSON FileLog 事件（`copy_plan`、`file_complete`、`progress_summary`），专为 Agent 和脚本程序化消费设计。
...
#### `file_complete` — 每个文件处理完成后触发（批量刷新时输出）
...
- `result`：`"done"` 或 `"error"`。当为 `"error"` 时，`errorCode` 包含错误信息。
- `skipped`：当文件因 mtime/size/ETag 匹配被跳过时，此字段为 `true`（仅 `--skip-by-mtime` 启用时，默认启用）。
