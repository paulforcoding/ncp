...
ncp 是一款对 AI Agent 友好、使用 DB 实现断点续传的海量文件复制工具。



## ncp解决了什么问题

ncp 主要针对的是当前企业有海量的文件和对象（OSS）难以被复制和迁移的问题，ncp 从三方面出发尝试去解决它：

一。使用 DB 来记录复制进度

	1. 在千万级文件/对象的场景下，遍历一次所有文件对 FS 和 OSS 都是严重的 IO 负担。ncp 使用 DB 来记录遍历出来的文件，实现了在一次复制任务中只遍历一次。
	1. 使用 DB 来记录文件/对象的复制进度。在千万级文件/对象的场景下，复制中断是常事，由于使用了 DB 来记录哪个文件是否被复制，所以断点续传变成了一件成本很低的事情，使得传统的海量文件复制任务变成可以分多次、逐步完成的过程。
	1. 保证性能。ncp 选用了写快读慢的本地 kv 存储来做 db 记录，如果复制任务一次都没有中断过，那么这个 db 是只写不读的，减少了 db 记录对复制性能的影响。另外，ncp 还采用并行、阶梯式 IO Size、支持directIO 等多种手段加速复制过程。

二。对 AI agent 友好，实现无人值守完成复制任务。

1. 海量数据迁移场景往往耗费数小时至几天，运维人员无法实时监控复制状况，最好的办法是让 AI agent 去监控复制进度，遇到问题才汇报给人类。ncp 结构化 NDJSON FileLog 事件（`file_complete`、`file_metadata_complete`、`progress_summary`），专为 Agent 和脚本程序化消费设计。使得 Agent 能知道复制进度和复制状态。
2. 配套 Skill 使得 Agent 能掌握 ncp 知识

三。数据可验证

海量数据迁移最大的问题是不知道数据是否复制完整。ncp 使用两个办法来保证数据完整。

1. 在跨服务器复制场景下，默认会使用md5 做数据比对。
2. ncp 把一个“源路径+目的路径”定义为一个复制任务，一个任务下面可以有多次“运行”，每次运行可以选择 copy 和 cksum 动作。所以一个复制任务可以先复制再比对，也可以先比对再复制，最终达到复制和比对都 100% 完成的状态。



## 支持的文件/OSS 类型

本地文件系统、远程 ncp 服务器（`ncp://`）、阿里云 OSS（`oss://`）、腾讯云 COS（`cos://`）、华为云 OBS（`obs://`）。
