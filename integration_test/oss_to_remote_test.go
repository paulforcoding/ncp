...
// TestIntegration_OSSToRemote_Copy covers matrix case #5 (OSS→Remote, copy, no-resume).
func TestIntegration_OSSToRemote_Copy(t *testing.T) {
	env := requireOSS(t)
	srcPrefix := newOSSPrefix(t, env, "oss2remote-copy-src")
	serveDir := t.TempDir()
...
// TestIntegration_OSSToRemote_Copy_Resume covers matrix case #6 (OSS→Remote, copy, resume).
func TestIntegration_OSSToRemote_Copy_Resume(t *testing.T) {
	env := requireOSS(t)
	srcPrefix := newOSSPrefix(t, env, "oss2remote-copy-r-src")
	serveDir := t.TempDir()
...
// TestIntegration_OSSToRemote_Cksum covers matrix case #7 (OSS→Remote, cksum, no-resume).
func TestIntegration_OSSToRemote_Cksum(t *testing.T) {
	env := requireOSS(t)
	srcPrefix := newOSSPrefix(t, env, "oss2remote-cksum-src")
	serveDir := t.TempDir()
...
// TestIntegration_OSSToRemote_Cksum_Resume covers matrix case #8 (OSS→Remote, cksum, resume).
func TestIntegration_OSSToRemote_Cksum_Resume(t *testing.T) {
	env := requireOSS(t)
	srcPrefix := newOSSPrefix(t, env, "oss2remote-cksum-r-src")
	serveDir := t.TempDir()
