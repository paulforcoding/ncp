...
// EnsureDirMtimeViaDst sets directory mtime through the Destination interface.
// Used when the destination is remote (ncp://), where direct filesystem access
// is not possible. Iterates DB in reverse order (deep→shallow) because writing
// files into a directory updates its mtime.
func EnsureDirMtimeViaDst(ctx context.Context, store progress.ProgressStore, src storage.Source, dst storage.Destination) error {
	it, err := store.Iter()
	if err != nil {
		return err
	}
	...
	for i := len(dirs) - 1; i >= 0; i-- {
		...
		_ = dst.SetMetadata(ctx, relPath, storage.FileAttr{
			Mtime: item.Attr.Mtime,
			Atime: item.Attr.Atime,
		})
	}
