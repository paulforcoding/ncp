...
| `ncp serve --listen <addr>` | 启动 ncp 协议服务器，为单次 task 服务（单模式），task 完成后立即退出（非闲置超时）；一个 ncp 客户端进程可开多个 replicator 并行连接。**注意：后续 cksum/resume 操作如果目标为 ncp://，需先重新启动 serve。** |
| `ncp task list` | 列出所有任务 |
| `ncp task show <taskID>` | 查看任务详情 |
| `ncp task delete <taskID>` | 删除任务 |
| `ncp config show` | 查看生效配置（AK/SK 自动脱敏） |
| `ncp config show --profile <name>` | 查看指定 profile 的配置 |
...
| `ncp://` | `ncp://server:9900/backup` | 远程 ncp 服务器（可作为源或目标） |
...
| `obs://` | `obs://prod@my-bucket/backup/` | 华为云 OBS（可作为源和目标）。**必须**带 `<profile>@` 前缀 |

**约束：**
- 源支持：本地、`ncp://`、`oss://`、`cos://`、`obs://`
- 目标支持：本地、`ncp://`、`oss://`、`cos://`、`obs://`
- `ncp cksum` 不支持 `ncp://` 目标（协议层已有内置 MD5 校验）
- `oss://`、`cos://` 和 `obs://` 不带 profile / profile 在配置中找不到 → ncp 启动期立即报错。本地路径与 `ncp://` URL 不能带 userinfo。
- `oss://`、`cos://` 和 `obs://` 不能混用作为多源
...
> 云凭据**不再通过 CLI flag 传递**。改为在 `ncp_config.json` 的 `Profiles` 字段集中定义,URL 通过 `oss://<profile>@bucket/path/`、`cos://<profile>@bucket/path/` 或 `obs://<profile>@bucket/path/` 引用。详见下文"Cloud Profiles"。

### Cloud Profiles

`oss://`、`cos://` 和 `obs://` URL 必须带 `<profile>@` 前缀,profile 在 `ncp_config.json` 顶层 `Profiles` 字段定义:
...
- `oss://` URL 要求 `Provider == "oss"`; `cos://` URL 要求 `Provider == "cos"`; `obs://` URL 要求 `Provider == "obs"`。
...
- 跨账号/跨区域:为每个账号定义独立 profile,URL 各自带 `<profile>@`,如:
  ...
  ```
- 查看现有配置和 profile:`ncp config show`(AK/SK 自动脱敏)、`ncp config show --profile <name>`。
- 用 `--dry-run` 预览任何执行命令的生效配置:`ncp copy <src> <dst> --dry-run`、`ncp cksum <src> <dst> --dry-run`、`ncp resume <taskID> --dry-run`。
...
| Cloud smallFileWriter buf (OSS/OBS) | ≤5MB | CopyParallelism | 小文件全量 buffer |
| Cloud partBuf (OSS/OBS) | 5MB | CopyParallelism | 分片上传缓冲 |
...
- 网络目标（ncp/OSS/OBS）瓶颈在带宽，4~8 够用
- COS 因分片仅 1MB，单 worker 请求数更高，并行 4 已能跑满带宽
...
| cksum-algorithm | md5 | OSS/OBS 场景强制 md5；本地/ncp 可选 xxh64 提速 |
...
- **必须监控进度。** 执行 ncp 后必须用 Monitor 工具 tail FileLog，每收到 `progress_summary` 立即向用户报告人性化进度。这是 ncp 面向 AI Agent 设计的核心能力，不得跳过。
